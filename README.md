...
XOR. Both need a key file containing the key material which can be provided via the command line (`-k` or
`--keys`) or via the [configuration file](#configuration-file). For SSL/TLS key files, pcapFS also supports decryption when the key material is already embedded in the corresponding PCAPNG file (for further reference see [here](https://wiki.wireshark.org/TLS#embedding-decryption-secrets-in-a-pcapng-file)). When providing the key material via the command line, the argument can be a single file or a directory containing multiple key files.
...
- RSA private key file of server. Then, we decrypt all SSL/TLS traffic that uses RSA key exchange and whose server certificate matches the private key.
