...
`--keys`) or via the [configuration file](#configuration-file). The argument can be a single file or a directory containing multiple key files.
Supported key file formats are the same as for Wireshark:
- key log files generated by applications when the environment variable `SSLKEYLOGFILE` is set. We support the format `CLIENT_RANDOM <ClientRandom> <MasterSecret>` and `RSA <first 8 bytes of encrypted preMasterSecret> <preMasterSecret>` for rsa key exchange.
