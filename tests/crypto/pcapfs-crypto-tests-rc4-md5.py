...
class TestRc4Md5:
    def test_with_key_file(self, test_pcap, expected_files_with_ssl):
        with mount_pcap(
            test_pcap, params=["-k", "{here}/keyfiles/rc4-md5.key".format(here=HERE)]
        ) as mountpoint:
    ...
    def test_read_processed_ssl_appdata(self, test_pcap, content_tls_appdata_all_plain):
        with mount_pcap(
            ...
            params=[
                "--show-all",
                "-k",
                "{here}/keyfiles/rc4-md5.key".format(here=HERE),
            ],
        ) as mountpoint:
            files = get_file_list(mountpoint)
            assert "ssl/0-1838_SSL" in files
...
def test_pcap():
    return os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "pcaps/rc4-md5.pcap"
    )
...
def content_tls_appdata_all_cipher():
    content_tls_appdata_all_cipher = (
        "0193362a0c21d85d0e6b8b4152e68603df80bd0a4a3f5b657d88fdb100ba78b8"
...
def content_tls_appdata_all_plain():
    content_tls_appdata_all_plain = (
        "474554202f20485454502f312e310a484f53543a206c6f63616c686f73740a0a"
