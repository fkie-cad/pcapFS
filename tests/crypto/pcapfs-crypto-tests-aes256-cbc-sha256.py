...
class TestAes256CbcSha256:
    def test_with_key_file(self, test_pcap, expected_files_with_ssl):
        with mount_pcap(
            test_pcap,
            params=["-k", "{here}/keyfiles/aes256-cbc-sha256.key".format(here=HERE)],
        ) as mountpoint:
    ...
    def test_read_processed_ssl_appdata(self, test_pcap, content_tls_appdata_all_plain):
        with mount_pcap(
            ...
            params=[
                "--show-all",
                "-k",
                "{here}/keyfiles/aes256-cbc-sha256.key".format(here=HERE),
            ],
        ) as mountpoint:
            files = get_file_list(mountpoint)
            assert "ssl/0-1726_SSL" in files
...
def test_pcap():
    return os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "pcaps/aes256-cbc-sha256.pcap"
    )
...
def content_tls_appdata_all_plain():
    content_tls_appdata_all_plain = (
        "68656c6c6f207468657265210a6865790a676f74746120676f2c20627965210a63750a"
    )
...
def content_tls_appdata_all_cipher():
    content_tls_appdata_all_cipher = (
        "6802793d4e1f0dce00b7e2e477fc39e0e8b7d0c03cd12b49ce3b77856d990b56"
