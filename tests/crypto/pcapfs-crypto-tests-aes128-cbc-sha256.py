...
class TestAes128CbcSha256:
    def test_with_key_file(self, test_pcap, expected_files_with_ssl):
        with mount_pcap(
            test_pcap,
            params=["-k", "{here}/keyfiles/aes128-cbc-sha256.key".format(here=HERE)],
        ) as mountpoint:
    ...
    def test_read_processed_ssl_appdata(self, test_pcap, content_tls_appdata_all_plain):
        with mount_pcap(
            ...
            params=[
                "--show-all",
                "-k",
                "{here}/keyfiles/aes128-cbc-sha256.key".format(here=HERE),
            ],
        ) as mountpoint:
            files = get_file_list(mountpoint)
            assert "ssl/0-1726_SSL" in files
...
def test_pcap():
    return os.path.join(
        os.path.dirname(os.path.realpath(__file__)), "pcaps/aes128-cbc-sha256.pcap"
    )
...
def content_tls_appdata_all_cipher():
    content_tls_appdata_all_cipher = (
        "fbe3f42cf1ccfdac9bcf8b4e09e5ef6bdffbae5d80c01cd3ab9597536d0f5926"
